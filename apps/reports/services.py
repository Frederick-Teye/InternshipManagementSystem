...
class ReportService:
    ...
    @staticmethod
    def generate_attendance_csv(queryset) -> HttpResponse:
        """
        Generate CSV export for attendance records.

        Args:
            queryset: Attendance queryset to export
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="attendance_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
        ...
        for attendance in queryset.select_related(
            "intern__user", "intern__branch", "approver"
        ):
            writer.writerow(
                [
                    attendance.check_in_time.strftime("%Y-%m-%d"),
                    attendance.intern.user.get_full_name(),
                    (
                        attendance.intern.branch.name
                        if attendance.intern.branch
                        else "N/A"
                    ),
                    ...
                    attendance.get_approval_status_display(),
                    (
                        attendance.approver.get_full_name()
                        if attendance.approver
                        else "N/A"
                    ),
                    (
                        attendance.approval_date.strftime("%Y-%m-%d %H:%M")
                        if attendance.approval_date
                        else "N/A"
                    ),
                ]
            )
    ...
    @staticmethod
    def generate_assessments_csv(queryset) -> HttpResponse:
        """
        Generate CSV export for assessment records.

        Args:
            queryset: PerformanceAssessment queryset to export
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="assessments_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
        ...
        for assessment in queryset.select_related("intern__user", "assessed_by__user"):
            writer.writerow(
                [
                    assessment.week_number,
                    assessment.intern.user.get_full_name(),
                    (
                        assessment.assessment_date.strftime("%Y-%m-%d")
                        if assessment.assessment_date
                        else "N/A"
                    ),
                    assessment.technical_skills or "N/A",
                    assessment.communication or "N/A",
                    assessment.teamwork or "N/A",
                    assessment.initiative or "N/A",
                    assessment.overall_rating or "N/A",
                    assessment.get_status_display(),
                    (
                        assessment.assessed_by.user.get_full_name()
